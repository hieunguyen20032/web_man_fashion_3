...
<?php 
    ...
    if( isset($_GET['code_order']) ) {
        ...
        $query = "SELECT * FROM tb_don_hang, tb_san_pham,tb_quan_huyen,tb_thanh_pho WHERE tb_thanh_pho.id_thanh_pho = tb_quan_huyen.id_thanh_pho && tb_don_hang.id_quan_huyen = tb_quan_huyen.id_quan_huyen && tb_san_pham.id_san_pham = tb_don_hang.id_san_pham && ma_don_hang='{$code_order}'";
...
<div class="add-order">
    <form name="frmadd-shiper" method="post" action="">
        ...
        <div class="row">
            <div class="col-xs-12">
                <div  class="info">
                    <h4 class="text-order">Đặt hàng #<span class="code_order"><?php echo $ma_don_hang; ?></span></h4>
                    <div class="row">
                        <div class="col-xs-6 left">
                            <div class="title-left">Chi tiết chung</div>
                            <div class="form-group wrapper-name">
                                <label>Họ và tên</label>
                                <input type="text" name="name" class="form-control" placeholder="Họ và tên" value="<?php echo $ten_khach_hang; ?>">
                                <div class="errors"></div>
                                <span class='icon-notify' style='right: 10px'></span>
                            </div>
                            <div class="form-group wrapper-email">
                                <label>Email</label>
                                <input type="text" name="email" class="form-control" placeholder="Không bắt buộc" value="<?php echo $email_khach_hang; ?>">
                                <div class="errors"></div>
                                <span class='icon-notify' style='right: 10px'></span>
                            </div>
                         <div class="form-group wrapper-phone_number">
                            <label>Số điện thoại</label>
                            <input type="text" name="phone_number" class="form-control" placeholder="Số điện thoại" value="<?php echo $dien_thoai_khach_hang; ?>">
                            <div class="errors"></div>
                            <span class='icon-notify' style='right: 10px'></span>
                        </div>  
                        <div class="form-group wrap-date">
                            ...
                            <?php 
                             // echo $order_day;
                                $order_day= date_create($ngay_don_hang);
                    ...
                    <div class="col-xs-6 right">
                        ...
                        <div class="form-group wrapper-tinhthanh">
                            ...
                            <select name="tinhthanh" class="select tinhthanh form-control">
                                ...
                                <?php 
                                    $query_tt = "SELECT * FROM tb_thanh_pho ORDER BY ma_thanh_pho";
                                      $result_tt = mysqli_query($dbc, $query_tt);
                                      while ( $rows = mysqli_fetch_assoc($result_tt) ) {
                                        ?>
                                        <option <?php echo $id_thanh_pho == $rows['id_thanh_pho'] ? 'selected="selected"' : '' ?> value="<?php echo $rows['id_thanh_pho'] ?>"> <?php echo $rows['ten_thanh_pho'] ?> </option>
                                        <?php
                                      }
                                ?>
                        ...
                        <div class="form-group wrapper-sonha-tenduong">
                            ...
                            <input type="text" name="sonha-tenduong" class="form-control" value=" <?php echo $dia_chi_khach_hang; ?>">
                            <div class="errors"></div>
                            <span class='icon-notify' style='right: 10px'></span>
                        </div> 
                    </div>
                ...
                <div class="products">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody class="wrap-product">
                            <?php 
                                $stt = 0;
                                $query = "SELECT tb_don_hang.id_san_pham, tb_san_pham.ma_san_pham, tb_san_pham.kich_co_san_pham kich_co_san_pham,tb_don_hang.kich_co_san_pham size_order,tb_don_hang.so_luong_san_pham, tb_san_pham.gia_khuyen_mai,tb_san_pham.anh_san_pham, tb_san_pham.ten_san_pham  FROM tb_don_hang, tb_san_pham WHERE tb_san_pham.id_san_pham = tb_don_hang.id_san_pham && ma_don_hang='{$code_order}'";
                                $result = mysqli_query($dbc, $query); 
                                while ( $rows =  mysqli_fetch_assoc($result) ) {
                                    extract($rows);
                                    $array_id_product[] =  $id_san_pham;
                            ?>
                            <tr class="<?php echo $id_san_pham; ?>">
                                <th style="vertical-align: middle;" class="stt_product"><?php echo $stt++; ?></th>
                                <td style="vertical-align: middle;"><?php echo $ma_san_pham; ?></td>
                                <td style="vertical-align: middle;"><?php echo $ten_san_pham; ?></td>
                                <td style="vertical-align: middle;"><img src="../<?php echo explode(" ", $anh_san_pham)[0]; ?>" width=50></td>
                                ...
                                <td style="vertical-align: middle;font-family: georgia" class="gia"><?php  echo number_format($gia_khuyen_mai, 0,',','.'); ?></td>
                                <td style="vertical-align: middle;">
                                    <input type="number" class="soluong" name="soluong" value="<?php echo $so_luong_san_pham; ?>" min="1" style="width: 60px;text-align: center;" class="form-control" >
                                </td>
                                <td style="vertical-align: middle;font-family: georgia" class="tong-tien"><?php echo number_format($gia_khuyen_mai*$so_luong_san_pham, 0,',','.'); ?></td>
...
<script type="text/javascript">
    ...
    $query="SELECT * FROM tb_san_pham ORDER BY id_san_pham";
    $result = mysqli_query($dbc, $query);
    ?>
    var availableTags = [
    <?php foreach ($result as $value) { 
        echo "{". 'value: '. "'" .$value['ma_san_pham']."-".$value['ten_san_pham']. "'" .",".'data: '. "'" . $value['id_san_pham'] . "'" ."}".",";
    } ?>
    ];
    var stt_product = 0;
         ...
         select: function(e, ui) {
            ...
            $.ajax({
                ...
                success: function(dt){
                    ...
                    var default_size = '';
                    var stt = 1;
                    for(key in dt['kich_co_san_pham']) {
                    ...
                    stt_product++;
                    $('.wrap-search').before(`
                        <tr class="`+dt['id_san_pham']+`">
                            <th style="vertical-align: middle;" class="stt_product">` + stt_product +`</th>
                            <td style="vertical-align: middle;">` + dt['ma_san_pham'] + `</td>
                            <td style="vertical-align: middle;">` + dt['ten_san_pham'] + `</td>
                            <td style="vertical-align: middle;"> <img width=50 src='../` + dt['anh_san_pham'].split(" ", 1) + `' /></td>
                            <td style="vertical-align: middle;" class="wrap_size"> <select name="size" class="size_product">` + str_size + `</select></td>
                            <td style="vertical-align: middle;font-family: georgia" class="gia">` + dt['gia_khuyen_mai'] + `</td>
...
<script type="text/javascript">
    ...
    var a = $('.tinhthanh').val();
        if($('.tinhthanh').val()){
             $.get("../xuli-tinhthanh/xuli-quanhuyen.php",{value:a,tb_quan_huyen: <?php echo $id_quan_huyen ?>},function(data){
                $('.select-districts').html(data);
            });
        }
    /* tinh thanh*/
    $('.tinhthanh').click(function(){
        var id_district = "<?php echo $id_quan_huyen ?>";
        var a = $('.tinhthanh').val();
        if(a ==""){
            return false;
        }
