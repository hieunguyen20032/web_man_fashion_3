...
<div class="row">
    <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
        ...
        {   
            ...
            if(empty($errors))
            {
                $query = "SELECT * FROM tb_thong_tin WHERE ten='gioithieu'";
                $result = mysqli_query($dbc, $query);
                if ( mysqli_num_rows($result) > 0 ) {
                    $query_ud = "UPDATE tb_thong_tin SET gia_tri='{$noidung}' WHERE ten='gioithieu'";
                    $result_ud = mysqli_query($dbc, $query_ud);
                    echo "<p class='results'> Lưu thành công </p>";
                } else {
                    $query_is = "INSERT INTO tb_thong_tin(ten, gia_tri) VALUES('gioithieu', '{$noidung}')";
