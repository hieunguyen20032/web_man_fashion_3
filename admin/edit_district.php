    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
            ...
            if (isset($_POST['submit'])) {
                ...
                $id_city = $_POST['city'];
                if (empty($errors)) {
                    $query_in = "UPDATE tb_quan_huyen SET ma_quan_huyen='$code', ten_quan_huyen ='$name',id_thanh_pho='$id_city'  where id_quan_huyen='$id'";
            ...
            //ket thuc submit
            $query = "SELECT * FROM tb_quan_huyen WHERE id_quan_huyen={$id}";
            ...
            <form name="frmadd-video" method="post">
                ...
                <h2 style=" color: red">Chỉnh sửa - quận "<?php echo $dong['ten_quan_huyen']?>"</h2>
                <div class="form-group">
                    <label>Mã quận, huyện</label>
                    <input type="text" name="code_district" value="<?php echo $dong['ma_quan_huyen']; ?>" class="form-control" placeholder='Mã thành phố'/>
                </div>

                <div class="form-group">
                    <label>Tên quận, huyện</label>
                    <input type="text" name="name_district" value="<?php echo $dong['ten_quan_huyen']; ?>" class="form-control" id="name_category" placeholder='Tên thành phố'/>
                </div>
				<div class="form-group">
	                ...
	                <select name="city" class="district" style="padding: 6px 12px;font-size: 14px;line-height: 1.42857143;color: #555;background-color: #fff;background-image: none;border: 1px solid #ccc;border-radius: 4px;">
	                    <?php 
	                        $query = "SELECT id_thanh_pho, ten_thanh_pho FROM tb_thanh_pho ORDER BY id_thanh_pho";
	                        $result = mysqli_query($dbc, $query);
	                        while ( $rows = mysqli_fetch_array($result, MYSQLI_NUM) ) {
	                    ?>
	                    <option value="<?php echo $rows[0]; ?>" <?php echo $rows[0] == $dong['id_thanh_pho'] ? 'selected="selected"' : ''; ?> > <?php echo $rows[1]; ?> </option>
	                    <?php    
	                        }
	                    ?>
	                </select>
