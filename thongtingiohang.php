			...
			<div class="col-xs-12 col-sm-6 left">
				<form>
						<div class="lienhe">
							...
							<div class="form-group height">
								<div class="payment" style="display:flex;gap 10;"> 
								<div class="col-xs-12 col-lg-4 nhan"><label class="name">Thanh toán *</label></div>
									<label class="pay"><input type="radio" value="Tiền Mặt" name="payment">Thanh Toán Khi Nhận Hàng</label>
									<label class="pay"><input type="radio" style="margin-left:20px" value="Chuyển Khoản" name="payment">Chuyển Khoản</label>
                        		</div>
							</div>
						</div>	
						<div class="diachi">
			...
			<div class="col-xs-12 col-sm-6 right">
				...
				<table class="table">
					...
					<?php 
						...
						?>
						<tr class="sp<?php echo $value['id_san_pham'] ?>">
							<td rowspan="
							<?php 
								$soluong=0; 
							...
							?>
							" class="sl-product">
                                <a class="img-thumbnail" href="product.php?id=<?php echo $value['id_san_pham'];?>">
								<?php
                                $img_product = explode(" ", $value['anh_thumb']);
							...
							<td>
								<strong class="name-product"><a href="product.php?id=<?php echo $value['id_san_pham']; ?>"><?php echo $value['ten_san_pham']; ?></a></strong>
							</td>
							<td class="sl-product<?php echo $value['id_san_pham']; ?>"><?php $soluong_product=0;
								foreach ($value['quantity'] as $sl_product) { $soluong_product+=$sl_product;} 
									echo $soluong_product ; ?></td>
							<td style="font-family: georgia;"><?php echo number_format($value['gia_khuyen_mai'], 0,',','.');?></td>
							<td style="font-family: georgia;" class="price<?php echo $value['id_san_pham']; ?>"><?php echo number_format($soluong_product *$value['gia_khuyen_mai'], 0,',','.');?></td>
							</tr>
							<?php 
									foreach ($value['quantity'] as $product_key => $product_val) { 
										
							?>
							<tr class="sp<?php echo $value['id_san_pham']; ?> xoa-sp" product="<?php
							echo $value['id_san_pham']; ?>">
								<td align="right">Size: <?php echo $product_key; ?></td>
								<td colspan="3">
									<input type="number" name="soluong"  value="<?php echo $product_val  ?>" min="1" max="10" style="width: 60px;text-align: center;" class="form-control" size="<?php echo $product_key; ?>" cart="<?php echo $value['id_san_pham']; ?>" val="<?php echo $product_val  ?>" price="<?php echo $value['gia_khuyen_mai'];?>">
								</td>
								<td>
	...
	$(document).ready(function(){
		...
		$(".guidonhang").click(function(){
			...
			if(errors.length==0){
				...
				if(payment_type === 'Chuyển Khoản'){
					// $.get('thanhtoanmomo.php', {
					// 	name: name,
					// 	email: email,
					// 	sdt: sdt,
					// 	tinh: tinh_thanh,
					...
					// });
					window.location.href = 'thanhtoanmomo.php?name=' + encodeURIComponent(name) + 
                           '&email=' + encodeURIComponent(email) + 
                           '&sdt=' + encodeURIComponent(sdt) + 
                           '&tinh=' + encodeURIComponent(tinh_thanh) + 
                           '&quan=' + encodeURIComponent(quan_huyen) + 
                           '&payment_type=' + encodeURIComponent(payment_type) + 
                           '&sonha=' + encodeURIComponent(sonha_tenduong) + 
                           '&phuong=' + encodeURIComponent(phuong_xa);
					...
					$.get('xuli/gui-don-hang.php', {
						...
						window.location.href = "xac-nhan-don-hang.php";
					});
				}
